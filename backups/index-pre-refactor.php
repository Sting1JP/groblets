...
<script>
// LocalStorage wrapper for Groblets
const Storage = {
  get: key => localStorage.getItem("groblets." + key),
  set: (key, value) => localStorage.setItem("groblets." + key, value),
  remove: key => localStorage.removeItem("groblets." + key),
  clearAll: () => {
    Object.keys(localStorage).forEach(k => {
      if (k.startsWith("groblets.")) localStorage.removeItem(k);
    });
  }
};
</script>
...
function handleActionClick(e) {
  ...
  const now = Date.now();
  const cooldownEnd = parseInt(Storage.get(cooldownKey) || "0", 10); 
  ...
  Storage.set(cooldownKey, newEnd.toString());
  updateAllCooldowns();
}
...
let mouseY = 0;
    ...
    let carePoints = parseInt(Storage.get("groblets.carePoints") || "0", 10);

    let milestoneFirstDone = !!Storage.get("groblets.milestoneFirstDone");
    let milestoneSecondDone = !!Storage.get("groblets.milestoneSecondDone");

    let spawnThreshold = parseInt(Storage.get("groblets.spawnThreshold") || "20", 10);


careDisplay.textContent = carePoints;
thresholdDisplay.textContent = spawnThreshold;
lastActionDisplay.textContent = Storage.get("groblets.lastAction") || "None";
...
function interact(points, label) {
  ...
  Storage.set("groblets.carePoints", carePoints);
  Storage.set("groblets.lastAction", label);

  // — Milestone: First (from 1 → 3)
  if (!milestoneFirstDone && grobs.length === 1) {
    milestoneFirstDone = true;
    Storage.set("groblets.milestoneFirstDone", "1");
  ...
  if (!milestoneSecondDone && grobs.length === 3) {
    ...
    Storage.set("groblets.milestoneSecondDone", "1");
  ...
  while (carePoints >= spawnThreshold) {
    ...
    Storage.set("groblets.spawnThreshold", spawnThreshold);
  }
...
function spawnSecondMilestoneGrobs() {
  ...
  grobCountDisplay.textContent = grobs.length;
  updateGrobStorage();
  Storage.set("groblets.tutorialDone", "1");
...
window.addEventListener('DOMContentLoaded', () => {
  const savedCount = parseInt(Storage.get("groblets.lastCount") || "0", 10);
  const lastTime = parseInt(Storage.get("groblets.lastTime") || "0", 10);
  const now = Date.now();
...
function updateGrobStorage() {
  Storage.set("groblets.lastTime", Date.now().toString());
  Storage.set("groblets.lastCount", grobs.length.toString());
}

function restoreGrobsFromStorage() {
  const count = parseInt(Storage.get("groblets.lastCount") || "0", 10);
  if (count > 1) {
    grobs = [];
...
function resetGrobletsSave() {
  if (confirm("Are you sure you want to reset all Groblets save data? This cannot be undone.")) {
    Storage.remove("groblets.carePoints");
    Storage.remove("groblets.spawnThreshold");
    Storage.remove("groblets.lastAction");
    Storage.remove("groblets.lastTime");
    Storage.remove("groblets.lastCount");

    // Remove milestone flags
    Storage.remove("groblets.milestoneFirstDone");
    Storage.remove("groblets.milestoneSecondDone");

    // Remove any obsolete flags
    Storage.remove("groblets.tutorialDone");
...
function checkOfflineReturnEvents(addedGrobs, hoursRaw, hoursDisplay, deaths = 0) {
  ...
  if (!Storage.get("groblets.tutorialDone") && addedGrobs <= 3) {
    const msgs = [
      "hey... ur back 😊",
...
function loadIdleGrobs() {
  const lastTime  = parseInt(Storage.get("groblets.lastTime")  || "0", "10");
  const lastCount = parseInt(Storage.get("groblets.lastCount") || "1", "10");
  const now       = Date.now();
...
if (!Storage.get("groblets.milestoneFirstDone") || !Storage.get("groblets.milestoneSecondDone")) {
...
function maybeStartRain() {
  const now = Date.now();
  const lastRainStart = parseInt(Storage.get("groblets.rain.lastStart") || 0);
  const nextRainStart = parseInt(Storage.get("groblets.rain.nextStart") || 0);
  const currentRainDuration = parseInt(Storage.get("groblets.rain.duration") || 0);
...
function scheduleNewRain() {
  ...
  Storage.set("groblets.rain.nextStart", nextRain.toString());
  Storage.set("groblets.rain.duration", duration.toString());
...
function triggerRainPersistent() {
  ...
  const now = Date.now();
  Storage.set("groblets.rain.lastStart", now.toString());
  ...
  const duration = parseInt(Storage.get("groblets.rain.duration") || 1800000);
...
function debugForceRainInOneMinute() {
  ...
  Storage.set("groblets.rain.nextStart", (now + delay).toString());
  Storage.set("groblets.rain.duration", duration.toString());
